...
1. 設上步驟中，沒被判為 $0$ 的列編號形成的集合為 $N$。
   ...
   * 否則從 $\mathbf{A'}$ 抽出對應的列得到 $\mathbf{A'\_N}$，並令 $\mathbf{B'\_L} = \mathbf{B'}\_{1:n; 1:\lfloor m/2\rfloor}, \mathbf{B'\_R} = \mathbf{B'}\_{1:n; \lfloor m/2\rfloor+1: m}$ (左半與右半)。接著令 $\mathbf{C'\_L} = \mathbf{A'\_N}\mathbf{B'\_L}, \mathbf{C'\_R} = \mathbf{A'\_N}\mathbf{B'\_R}$，並遞迴計算 $\mathbf{C'\_L}$ 和 $\mathbf{C'\_R}$。
...
定義前綴和 $S^1\_i = \sum\_{k=1}^ib_k$，如此一來 $\sum_{k=p}^cb_k$ 便能簡化成 $S^1\_c-S^1\_{p-1}$。
