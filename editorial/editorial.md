...
這裡用豎線與頂線代表左右邊與上面的閘門。以上兩圖為例，如果在地圖的最上方與最左方留下一列及一行 $1$，可以透過操作閘門來將任意一個數字 $x$ 變成 $2x$ 或者 $2x+1$。依照上述的方法將 $T$ 拆成 $2$ 進位來構造可以得到一個 $n+m = 121$ 左右的解，得分為 $38$ 分。
